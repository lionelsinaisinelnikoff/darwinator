...
def seed_demo(db):
    c = db.cursor()
    admin_hash = generate_password_hash("stern2026")
    c.execute(
    ...
    students = [
        "rafiki_mba", "elisa_innovate", "lionel_strategist",
        "cintia_esg", "alex_vc", "sara_product", "omar_energy"
    ]
    student_ids = []
    for s in students:
        h = generate_password_hash("demo2026")
        c.execute(
    ...
    c.execute("""
        INSERT INTO tournaments (code, name, description, current_round, round1_open, round2_open)
        VALUES (?, ?, ?, 1, 1, 0)
    """, (
        "NSAD-TPMI-F25-R1",
    ...
    # Assign sample ideas only to the last few demo accounts so primary test users start clean
    for i, (name, desc) in enumerate(sample_ideas):
        uid = student_ids[-(i % 3) - 1]  # omar_energy, sara_product, alex_vc
        c.execute(
            "INSERT INTO ideas (tournament_id, user_id, round, name, description) VALUES (?, ?, 1, ?, ?)",
            (tid, uid, name, desc)
        )
    ...
    for iid in idea_ids:
        for sid in student_ids[:4]:
            ...
            c.execute(
                "INSERT OR IGNORE INTO evaluations (idea_id, user_id, rating, comment) VALUES (?, ?, ?, ?)",
...
def get_unrated_ideas(user_id, tournament_id, round_num=1, limit=200):
    """Return ideas the user has not yet rated, excluding their own submissions."""
    return get_db().execute("""
        ...
        LIMIT ?
    """, (tournament_id, round_num, user_id, user_id, limit)).fetchall()
...
def dashboard():
    ...
    my_ratings = db.execute(
        "SELECT COUNT(*) FROM evaluations WHERE user_id = ?", (user_id,)
    ).fetchone()[0]
    total_r1 = db.execute(
        "SELECT COUNT(*) FROM ideas WHERE tournament_id = ? AND round = 1",
        (tournament["id"],)
    ).fetchone()[0]
    unrated = len(get_unrated_ideas(user_id, tournament["id"], 1, 500))
    round2_ideas = get_user_ideas(user_id, tournament["id"], 2) if tournament["current_round"] >= 2 else []

    return render_template(
        "dashboard.html",
        tournament=tournament,
        my_ideas_r1=my_ideas_r1,
        my_ratings=my_ratings,
        total_r1=total_r1,
        unrated=unrated,
        round2_ideas=round2_ideas,
...
def submit():
    if session.get("role") == "admin":
        flash("Admins manage the tournament — only students submit ideas.", "info")
    ...
    if request.method == "POST":
        ...
        count = db.execute(
            "SELECT COUNT(*) FROM ideas WHERE user_id = ? AND tournament_id = ? AND round = 1",
...
def evaluate():
    if session.get("role") == "admin":
        flash("Admins manage the tournament — only students evaluate ideas.", "info")
        return redirect(url_for("admin"))
    tournament = get_tournament()
    user_id = session["user_id"]
    db = get_db()
            ...
            if key.startswith("rating_"):
                try:
                    ...
                    if 0 <= rating <= 10:
                        ...
                        db.execute("""
                            INSERT OR REPLACE INTO evaluations (idea_id, user_id, rating, comment)
                            VALUES (?, ?, ?, ?)
    ...
    unrated = get_unrated_ideas(user_id, tournament["id"], round_num, limit=200)
    my_count = db.execute("""
        SELECT COUNT(*) FROM evaluations e
        JOIN ideas i ON e.idea_id = i.id
        WHERE e.user_id = ? AND i.round = ?
    """, (user_id, round_num)).fetchone()[0]
    # Total = all ideas in this round that are NOT the user's own
    total = db.execute(
        "SELECT COUNT(*) FROM ideas WHERE tournament_id = ? AND round = ? AND user_id != ?",
        (tournament["id"], round_num, user_id)
    ).fetchone()[0]
...
def edit_idea(idea_id):
    if session.get("role") == "admin":
        flash("Admins manage the tournament — only students edit their ideas.", "info")
        return redirect(url_for("admin"))
    tournament = get_tournament()
    if not tournament["round1_open"]:
        flash("Round 1 is closed — ideas can no longer be edited.", "warning")
        return redirect(url_for("dashboard"))
    db = get_db()
    idea = db.execute(
        "SELECT * FROM ideas WHERE id = ? AND user_id = ? AND round = 1",
        (idea_id, session["user_id"])
    ).fetchone()
    if not idea:
        flash("Idea not found or you do not own it.", "danger")
        return redirect(url_for("submit"))
    if request.method == "POST":
        name = request.form.get("name", "").strip()
        description = request.form.get("description", "").strip()
        ...
        db.execute(
            "UPDATE ideas SET name = ?, description = ? WHERE id = ? AND user_id = ?",
            (name, description, idea_id, session["user_id"])
        )
        db.commit()
        flash(f"Idea “{name}” updated.", "success")
        return redirect(url_for("submit"))
    return render_template("edit_idea.html", tournament=tournament, idea=idea)
...
def delete_idea(idea_id):
    ...
    tournament = get_tournament()
    if not tournament["round1_open"]:
        flash("Round 1 is closed — ideas can no longer be deleted.", "warning")
        return redirect(url_for("dashboard"))
    db = get_db()
    idea = db.execute(
        "SELECT * FROM ideas WHERE id = ? AND user_id = ? AND round = 1",
    ...
    if idea:
        db.execute("DELETE FROM evaluations WHERE idea_id = ?", (idea_id,))
...
def round2():
    if session.get("role") == "admin":
        flash("Admins manage the tournament — only students advance solutions.", "info")
        return redirect(url_for("admin"))
    tournament = get_tournament()
    ...
    if request.method == "POST":
        ...
        existing = db.execute(
            "SELECT id FROM ideas WHERE user_id = ? AND tournament_id = ? AND round = 2",
            (user_id, tournament["id"])
        ).fetchone()
        if existing:
        ...
        db.execute("""
            INSERT INTO ideas (tournament_id, user_id, round, name, description, slides_url)
            VALUES (?, ?, 2, ?, ?, ?)
...
def admin():
    ...
    stats = {
        ...
        "ideas_r1": db.execute(
            "SELECT COUNT(*) FROM ideas WHERE tournament_id=? AND round=1", (tournament["id"],)
        ).fetchone()[0],
        "evaluations": db.execute("""
            SELECT COUNT(*) FROM evaluations e
            JOIN ideas i ON e.idea_id = i.id WHERE i.tournament_id = ?
        """, (tournament["id"],)).fetchone()[0],
        "avg_rating": db.execute("""
            SELECT COALESCE(AVG(e.rating), 0) FROM evaluations e
            JOIN ideas i ON e.idea_id = i.id WHERE i.round = 1 AND i.tournament_id = ?
        """, (tournament["id"],)).fetchone()[0],
    }

    top_ideas = db.execute("""
        SELECT i.*, u.username as submitter,
               COUNT(e.id) as num_ratings,
               COALESCE(AVG(e.rating), 0) as avg_rating
        FROM ideas i
        JOIN users u ON i.user_id = u.id
        LEFT JOIN evaluations e ON i.id = e.idea_id
        WHERE i.tournament_id = ? AND i.round = 1
        GROUP BY i.id
    ...
    if tournament["current_round"] >= 2:
        round2_solutions = db.execute("""
            ...
            JOIN users u ON i.user_id = u.id
            LEFT JOIN evaluations e ON i.id = e.idea_id
            WHERE i.tournament_id = ? AND i.round = 2
            GROUP BY i.id
    ...
    return render_template(
        ...
        stats=stats,
        top_ideas=top_ideas,
        round2_solutions=round2_solutions
    )
...
def advance_round():
    ...
    if new_round == 2:
        db.execute(
            "UPDATE tournaments SET current_round=2, round1_open=0, round2_open=1 WHERE id=?",
            (tournament["id"],)
        )
        flash("Round 1 closed. Round 2 is now open for advanced solutions.", "success")
    elif new_round == 3:
        db.execute(
            "UPDATE tournaments SET current_round=3, round2_open=0 WHERE id=?",
            (tournament["id"],)
        )
...
def export():
    ...
    df = pd.read_sql_query("""
        SELECT i.id, i.round, i.name, i.description, i.slides_url,
        ...
        GROUP BY i.id
        ORDER BY i.round, avg_rating DESC
